...
os.chdir('..')
parent = os.getcwd()
# data_root = parent+'/sim_data/blinded_holes/'
data_root = parent+'/sim_data/non_blinded_holes_new/'
save_img = False
verbose = True
...
seps = np.array([5, 20])
# heights = np.array([-25, -10, -5, 0, 5, 10, 25])
heights = np.array([-25.0, -10.0, -5.0, 0.0, 5.0, 10.0, 25.0])
...
def build_attractor(results_dir, N, hr, dist_from_edge, root=data_root):
    ...
    attractor.build(lambda pos: nholes(pos[0], pos[1], pos[2], R=R, hr=hr, from_edge=dist_from_edge), N, cyl_sym=False) # lambda to pass other params
...
def sim_main(params):
    ...
    sim_start = time.time()
    comp_times = Parallel(n_jobs=ncores)(delayed(rotary_simulation)(param) for param in tqdm(param_list))
    sim_end = time.time()
...
if __name__ == '__main__':
    ...
    N = 7
    hr = 25.
    dist_from_edge = 18.18
    
    full_start = time.time()
    
    sim_main((N, hr, dist_from_edge))
