...
seps = np.array([5, 20, 50])
heights = np.array([5])
lambdas = np.array([1e-6, 10e-6, 50e-6])
...
if __name__ == '__main__':
    ...
    dist_from_edge = np.array([2.5, 5, 10, 15])
    Ns = np.array([5, 7, 10])
    hrs = np.array([2.5, 5, 12.5, 25])
    
    param_list = list(itertools.product(Ns, hrs, dist_from_edge))
    
    full_start = time.time()
    Parallel(n_jobs=24)(delayed(sim_main)(param) for param in param_list)
    full_end = time.time()
