...
os.chdir('..')
parent = os.getcwd()
data_root = parent+'/sim_data/square_slots_new/'
save_img = True
verbose = True
...
def build_attractor(results_dir, N, hr, from_edge, root=data_root):
    # pretty self-explainatory
    if os.getcwd() != root+results_dir:
        os.chdir(root+results_dir)
    
    attractor = attractor_profile(R, height, Rb=Rb, rhob=rhob)
    attractor.build(lambda pos: slots_squares(pos[0], pos[1], pos[2], R=R, angular=2*hr, radial=2*hr, from_edge=from_edge), N, cyl_sym=True) # lambda to pass other params
...
def sim_main(params):
    ...
    N, hr, from_edge = params
    
    results_dir = 'N=' + str(N) + '_radius=' + str(hr) + '_edge=' + str(from_edge)
    
    attractor = find_attractor(results_dir)
    if attractor == None:
        attractor = build_attractor(results_dir, N, hr, from_edge)
