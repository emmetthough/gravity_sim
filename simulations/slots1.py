...
os.chdir('..')
parent = os.getcwd()
data_root = parent+'/sim_data/slots1/'
save_img = False
verbose = True
...
from_edge = 2.5e-6
def slots(r,phi,z,R=R,angular=25,radial=25):
    ...
    if (R - from_edge - r) < radial and np.abs(phi) < dphi/2:
        return 0.
...
def build_attractor(results_dir, N, angular, radial, root=data_root):
    # pretty self-explainatory
    if os.getcwd() != root+results_dir:
        os.chdir(root+results_dir)
    
    attractor = attractor_profile(R, height, Rb=Rb, rhob=rhob)
    attractor.build(lambda pos: slots(pos[0], pos[1], pos[2], R=R, angular=angular, radial=radial), N, cyl_sym=True) # lambda to pass other params
...
def sim_main(params):
    ...
    N, angular, radial = params
    
    results_dir = 'N=' + str(N) + '_angular=' + str(angular) + '_radial=' + str(radial)
    
    attractor = find_attractor(results_dir)
    if attractor == None:
        attractor = build_attractor(results_dir, N, angular, radial)
...
if __name__ == '__main__':
    ...
    Ns = np.array([7])
    angulars = np.linspace(2.5, 25, 7)
    radials = np.concatenate((np.linspace(2.5, 25, 7), np.array([80])))
    
    param_list = list(itertools.product(Ns, angulars, radials))
    
    full_start = time.time()
    Parallel(n_jobs=28)(delayed(sim_main)(param) for param in param_list)
    full_end = time.time()
